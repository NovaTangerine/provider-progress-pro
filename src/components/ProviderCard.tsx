      ...
      className={`group/card ${isFocused ? 'group/card--focused' : ''} rounded-lg border border-border bg-card shadow-sm ${anyCardFocused && !isFocused ? 'pointer-events-none' : 'hover:shadow-card-hover hover:border-foreground/20 outline outline-0 hover:outline-[1px] outline-foreground/10 -outline-offset-1'} transition-[box-shadow,border-color,outline-width] duration-200 overflow-hidden ${isFocused ? 'relative z-[60] ring-2 ring-primary/30' : ''}`}>
          ...
          <div className="space-y-2">
            ...
            <Collapsible open={availabilityExpanded}>
              <CollapsibleContent className="overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
                <div className="rounded-md bg-[hsl(0,0%,97.5%)] border-[1.5px] p-6 space-y-4 mt-1" style={{ borderImage: 'linear-gradient(to bottom, hsl(0,0%,96%), hsl(0,0%,90%)) 1', borderRadius: '0.375rem' }}>
        ...
        <div className={`px-7 ${highlights.length > 0 ? 'border-t border-dashed border-border/[0.72] relative' : ''} pt-[24px] pb-[16px]`}>
          ...
          <>
              ...
              className={`rounded-md bg-[hsl(0,0%,97.5%)] border-[1.5px] group-hover/card:bg-[hsl(230,12%,97.5%)] p-3 py-5 space-y-2 transition-[background-color] duration-[480ms] ${hasMore ? "cursor-pointer hover:!bg-[hsl(230,12%,96%)]" : ""}`}
              style={{ borderImage: 'linear-gradient(to bottom, hsl(0,0%,96%), hsl(0,0%,90%)) 1' }}>
