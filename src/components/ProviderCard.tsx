...
export function ProviderCard({ provider, highlightsExpanded, onHighlightsToggle, availabilityExpanded, onAvailabilityToggle, constrainHeight, focusModeActive, isFocused, onFocus, onExitFocus }: ProviderCardProps) {
  ...
  return (
    <>
      ...
      className={`group/card ${isFocused ? 'group/card--focused' : ''} rounded-lg border border-border bg-card shadow-sm ${focusModeActive && !isFocused ? '' : 'hover:shadow-card-hover hover:border-foreground/20 outline outline-0 hover:outline-[1px] outline-foreground/10 -outline-offset-1'} transition-[box-shadow,border-color,outline-width] duration-200 overflow-hidden ${isFocused ? 'relative z-[60] ring-2 ring-primary/30' : ''} ${focusModeActive && !isFocused ? 'opacity-60 cursor-pointer' : ''}`}>
        {/* Header */}
        ...
        <div className={`px-7 ${highlights.length > 0 ? 'border-t border-dashed border-border/[0.72] relative' : ''} pt-[24px] pb-[16px]`}>
          ...
          <>
              ...
              className={`rounded-md bg-[hsl(0,0%,97.5%)] group-hover/card:bg-[hsl(230,12%,97.5%)] p-3 py-5 space-y-2 transition-[background-color] duration-[480ms] ${hasMore ? "cursor-pointer hover:!bg-[hsl(230,12%,96%)]" : ""}`}>
