  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-background max-w-[1440px] mx-auto">
      ...
      <>
        ...
        <div className="group/grid pt-[calc(1.5rem+8px)] p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-6 animate-fade-in relative" data-focus-active={focusedProviderId ? "true" : undefined} style={{ gridTemplateRows: cardSyncMode ? 'auto' : undefined, gridAutoRows: cardSyncMode ? 'auto' : undefined }}>
          {filteredProviders.map((provider) =>
            <ProviderCard
              key={provider.id}
              provider={provider}
